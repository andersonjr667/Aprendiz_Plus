...
class JobRecommendationAI {
  ...
  getProfileSummary() {
    ...
    // Usuários logados começam com 30% de base
    let profileStrength = 30;
    
    // Dados básicos (30% adicional)
    ...
    // Interações (30% adicional)
    if (totalInteractions > 0) profileStrength += 8;
    if (totalInteractions > 5) profileStrength += 8;
    if (favorites > 0) profileStrength += 7;
    if (searches > 0) profileStrength += 7;
    
    // Diversidade (10% adicional)
    if (categorias > 0) profileStrength += 4;
    if (categorias > 2) profileStrength += 3;
    if (categorias > 5) profileStrength += 3;
